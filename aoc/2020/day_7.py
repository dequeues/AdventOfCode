...
def get_tree(data: list[str]) -> dict[str, str]:
    rules = {}
    for line in data:
...
def has_bag(color, target, tree) -> bool:
    contents = tree[color]
    return any(inner == target or
               has_bag(inner, target, tree) for _, inner in contents
               )
...
def count_bags_inside(color, tree) -> int:
    ...
    return (
            sum(
                n * (1 + count_bags_inside(inner, tree))
                for n, inner in contents
              )
    )
